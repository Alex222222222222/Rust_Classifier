...
impl Classifier {
    ...
    pub fn add_document_tokenized(&mut self, document: &[String], label: &String) {
        ...
        let classification = self.classifications.get_mut(label).unwrap();
    ...
    pub fn add_documents_tokenized(&mut self, examples: &[(Vec<String>, String)]) {
        for (document, label) in examples.iter() {
    ...
    pub fn get_document_probabilities_tokenized(&self, document: &[String]) -> Vec<(String, f64)> {
        let all_probs: Vec<(String, f64)> = self
            .classifications
            .values()
            .map(|classification| {
                let score = classification.score_document(document, &self.vocab);
        ...
        all_probs
            .into_iter()
            .map(|(c, s)| (c, 1.0 - s / total_prob))
            .collect()
    }

    /// Similar to classify but instead of returning a single label, returns all
    /// labels and the probabilities of each one given the document
    pub fn get_document_probabilities(&self, document: &str) -> Vec<(String, f64)> {
    ...
    pub fn to_json(&self) -> Result<String, crate::Error> {
        match serde_json::to_string(self) {
            Ok(json) => Ok(json),
            Err(err) => Err(crate::Error::EncodeJsonError(err.to_string())),
        }
    }

    /// Builds a new classifier from a JSON string
    pub fn from_json(encoded: &str) -> Result<Classifier, crate::Error> {
        match serde_json::from_str(encoded) {
            Ok(classifier) => Ok(classifier),
            Err(err) => Err(crate::Error::ParseJsonError(err.to_string())),
        }
    }
}

impl Classification {
    fn new(label: &str) -> Classification {
        Classification {
    ...
    fn train(&mut self, vocab: &HashSet<String>, total_examples: u32, smoothing: f64) {
        ...
        self.default_word_probability =
            smoothing / (self.num_words as f64 + smoothing * vocab.len() as f64);

        for word in vocab.iter() {
            if self.words.contains_key(word) {
                let word_entry = self.words.get_mut(word).unwrap();
                let word_count = word_entry.0;
                let p_word_given_label = (word_count as f64 + smoothing)
    ...
    fn score_document(&self, document: &[String], vocab: &HashSet<String>) -> f64 {
...
fn split_document(document: &str) -> Vec<String> {
